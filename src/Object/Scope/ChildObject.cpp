...
bool ChildObject::Begin()
{
	...
	if (m_frame)
		value = ImGui::BeginChildFrame(ImGui::GetID(p_name.c_str()), p_size, m_flags);
	else
		value = ImGui::BeginChild(p_name.c_str(), p_size, m_hasBorder, m_flags);
...
void ChildObject::DisplayOnInspector()
{
	...
	ImGui::CheckboxFlags("No background", &m_flags, ImGuiWindowFlags_NoBackground);
	ImGui::CheckboxFlags("No title bar", &m_flags, ImGuiWindowFlags_NoTitleBar);
	ImGui::CheckboxFlags("No resize", &m_flags, ImGuiWindowFlags_NoResize);
	ImGui::CheckboxFlags("No move", &m_flags, ImGuiWindowFlags_NoMove);
	ImGui::CheckboxFlags("No collapse", &m_flags, ImGuiWindowFlags_NoCollapse);
	ImGui::CheckboxFlags("No scrollbar", &m_flags, ImGuiWindowFlags_NoScrollbar);
	ImGui::CheckboxFlags("No scroll with mouse wheel", &m_flags, ImGuiWindowFlags_NoScrollWithMouse);
...
void ChildObject::Serialize(std::string& content) const
{
	...
	content += "if (";
	if (m_frame)
		content += "ImGui::BeginChildFrame(ImGui::GetID(\"" + p_name + "\"), ImVec2(" + std::to_string(size.x) + ", " + std::to_string(size.y) + "))";
	else
